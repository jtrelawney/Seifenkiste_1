...
int tcp_class::TCP_receive_message(buffer_class &message){

    int received_message_length;
    int target_length = message.size();
    std::cout << "tcp class : target message length = " << target_length << std::endl;
    if (target_length == 0) received_message_length = read(accepted_connection_fd, message.data(), TCP_MAX_BUFFER_SIZE);
    else received_message_length = read(accepted_connection_fd, message.data(), target_length);
    
    message.resize(received_message_length);
    
    std::cout << "tcp class : receive message length = " << received_message_length << std::endl;
    //std::cout << "\n\nmessage received, size = " << message.size() << "   " << message << std::endl;
...
int tcp_class::TCP_send_message(buffer_class buffer){
    std::cout << "tcp_class : TCP - transmitting buffer_class object, length = " << buffer.size() << std::endl;
    ...
    std::cout << "message size = " << buffer.size() << "    message = " << buffer << std::endl;
    
    int n = write(socket_fd,buffer.data(),buffer.size());
    if (n < 0) 
