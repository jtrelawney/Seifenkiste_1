...
cockpit_class::cockpit_class(const address_class &whoami) : event_counter_(0), whoami_(whoami) {
...
void cockpit_class::keep_processing(){
    ...
    int reader_index = whoami_.get_process_index();
    std::time_t ct;
    ...
    std::unique_lock<std::mutex> lock_communication_mutex(G_QUEUE_COORDINATION_VARS_OBJ.message_available_mutex[reader_index]);
    while(G_END_FLAG.read_flag()==false) {
        event_counter_++;

        //std::cout << "\n\n===========================================" << std::endl;
        //std::cout << "cockpit releases its mutex and sleeps" << std::endl;
        G_QUEUE_COORDINATION_VARS_OBJ.message_available_condition[reader_index].wait(lock_communication_mutex);
        ...
        if ( G_QUEUE_COORDINATION_VARS_OBJ.message_available_flag[reader_index] == true ) {
            ct = get_time();
            //std::cout << "cockpit (pid) = " << reader_index << " wakes up @ " << ct << std::endl;
            ...
            G_QUEUE_COORDINATION_VARS_OBJ.message_available_flag[reader_index] = false;
    ...
    ct = get_time();
    std::cout << "\ncockpit pid = " << whoami_ << " verified the shutdown flag and is exiting @ " << ct << " with count = " << event_counter_ << std::endl;
}
